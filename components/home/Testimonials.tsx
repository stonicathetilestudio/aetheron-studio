    ...
    return (
        <section className="py-24 sm:py-32 overflow-hidden bg-transparent relative">
            ...
            <div className="relative w-full overflow-hidden mask-gradient-x">
                ...
                <div className="absolute left-0 top-0 bottom-0 z-20 w-32 bg-gradient-to-r from-black to-transparent" />
                <div className="absolute right-0 top-0 bottom-0 z-20 w-32 bg-gradient-to-l from-black to-transparent" />

                <div className="flex w-max gap-8 animate-marquee hover:[animation-play-state:paused] py-10">
                    {[...testimonials, ...testimonials].map((testimonial, index) => (
                        <div
                            key={index}
                            className="relative w-[350px] md:w-[450px] flex-shrink-0 rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm transition-all hover:bg-white/10 hover:border-primary/30 group"
                        >
                            <Quote className="absolute top-8 right-8 h-8 w-8 text-white/5 group-hover:text-primary/20 transition-colors" />
